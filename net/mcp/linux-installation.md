...
url: annotation/net/mcp/linux-installation
title: Install on Linux
weight: 3
description: "Install the GroupDocs.Annotation MCP server on Linux with the setup bootstrapper (installs PowerShell, Docker or the .NET 10 SDK with libgdiplus and libfontconfig1) and the guided installer."
keywords: install MCP server Linux, MCP server Ubuntu, libgdiplus MCP error, self-hosted document annotation Linux
productName: GroupDocs.Annotation MCP Server for .NET
toc: True
---

Install the GroupDocs.Annotation MCP server on **Linux** with the setup bootstrapper — it installs PowerShell (`pwsh`) plus Docker, or the .NET 10 SDK with `libgdiplus` and `libfontconfig1` — and then the guided installer:
...
pwsh ./install-groupdocs-mcp.ps1 -Channel docker \
  -Products annotation -Clients claude-desktop -Verify
```
...
Client config locations on Linux: Claude Desktop `~/.config/Claude/claude_desktop_config.json`, VS Code user-level `~/.config/Code/User/mcp.json` — full list in [Register in AI clients]({{< ref "annotation/net/mcp/install-in-ai-clients.md" >}}).
...
A passing run completes the MCP handshake (eleven tools listed) and — when a document sits in your storage folder — a real `get_document_info` call. Headless servers: the docker channel plus `-EmitCompose` gives you a `docker-compose.yml` for supervised operation.
...
* **Fonts look wrong in annotated documents and exports** — install `ttf-mscorefonts-installer` (the setup script pre-accepts its EULA via debconf).

Installing on a different OS? [Windows]({{< ref "annotation/net/mcp/windows-installation.md" >}}) · [macOS]({{< ref "annotation/net/mcp/macos-installation.md" >}})
