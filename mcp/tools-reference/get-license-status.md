...
url: annotation/mcp/tools-reference/get-license-status
...
`get_license_status` reports how this server is licensed and, under metered licensing, how much has been consumed. It takes no arguments, processes no document, and changes nothing. Example prompt: *"Is the annotation server licensed?"*
...
| `server` / `engine` | MCP server version and the underlying GroupDocs.Annotation engine version |
| `note` | Why licensing did not end up where you asked — present only when something needs attention |

A rejected key pair does not fail silently: the mode reverts to `evaluation` and `note` carries the reason, so you learn it here rather than from a trial badge on your annotated document. See [Licensing]({{< ref "annotation/mcp/getting-started/licensing.md" >}}).

## Example prompts

* *"Is the annotation server licensed?"*
* *"How much metered credit have I used?"*
* *"Which GroupDocs.Annotation engine version is running?"*
