---
id: mcp-licensing
url: annotation/mcp/getting-started/licensing
title: Licensing
weight: 7
description: "The GroupDocs.Annotation MCP server runs in evaluation mode out of the box; unlock it with your existing GroupDocs.Annotation license file or with metered (pay-per-use) keys, and confirm the active mode with the get_license_status tool."
...
| **Evaluation** | nothing | A trial badge is stamped at the top of every page |
...
```json
{ "licensePath": "D:/Storage/Licenses/GroupDocs.Annotation.lic" }
```

**Manual (NuGet channel)** — set the environment variable in your client's server entry:

```json
"env": { "GROUPDOCS_LICENSE_PATH": "/path/to/GroupDocs.Annotation.lic" }
```
...
{{< alert style="warning" >}}
**Both keys are required.** With only one set, the server ignores the metered configuration and stays in evaluation mode — it reports this explicitly rather than failing silently. Check with [`get_license_status`]({{< ref "annotation/mcp/tools-reference/get-license-status.md" >}}).
{{< /alert >}}
...
Ask your agent *"what is the license status of the annotation server?"*. The [`get_license_status`]({{< ref "annotation/mcp/tools-reference/get-license-status.md" >}}) tool (server **26.9.0+**) answers without processing a document:
...
```json
{
  ...
  "server": { "name": "GroupDocs.Annotation.Mcp", "version": "26.9.0" },
  "engine": { "name": "GroupDocs.Annotation", "version": "26.6.0.0" }
}
```
...
* [Purchase GroupDocs.Annotation](https://purchase.groupdocs.com/pricing/annotation/net) · [Product page](https://products.groupdocs.com/annotation/net/)
